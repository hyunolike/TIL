# Today I Learned 🤗

[![블로그 방문하기](https://img.shields.io/badge/Blog-hyunolike.github.io-blue?style=for-the-badge)](https://hyunolike.github.io/)

> 오늘 새로 배운 것을 다음의 규칙으로 commit 한다.

## 작성규칙
- 문서 생성은 GFM(Github Falvored Markdown)을 사용한다.(확장자 `.md`)
- 언어나 기술명은 폴더로 만든다. (root에 문서를 만들지 않는다.)
- 파일명은 __영어__ 로 한다.
- 해당 문서를 다시 봤을 때, 추가적인 검색의 비용이 들지 않도록 자세히 기록한다.
- 더 많은 공유를 원하는 기록은 블로그에 포스팅한다.
- 쉴 땐 확실히 쉬고, contributions를 초록색으로 채우는 것에 집착하지 않는다. <br>
    ㄴ 순전히 나를 위한 공간
- reference를 명시하고, 원작자가 참고를 허용하는 자료만 사용한다.

## 👨‍💻 메인 저장소
> 새로 배운 내용 및 공부 정리하는 곳 :)

### VCS
- [Git Mistakes](VCS/Git.md) 
- [Git-flow](VCS/Git-flow.md)
- [Commit Message 규칙](VCS/commit-message.md)

### 대학교 전공수업
- [인공지능](/University/ai.md) `마지막 대학수업😋`
- [JSP/Servlet](/University/jsp_servlet.md)
### Java
- [java 문법](/Java/Java_Basic.md)
- [Design Pattern](Java/DesignPattern.md)
### JavaScript
- [JavaScript 문법](/JavaScript/JavaScript_Basic.md)
### React
- [Redux](#)


## 🏡 다락방 저장소
> 세미나 및 기타 참고하는 자료 저장하는 곳 :)

### 세미나
- [통합 세미나](/Seminar/integrated_seminar.md)
- [웹 세미나](#)
- [서버 세미나](/Seminar/server_seminar.md)

### ETC
- [🍞빵 부스러기](#)
